  ...
  def show
    ...
    if (params.keys & expected).any? # Checks if at least 1 param was passed
      if (params.keys & expected.last(2)).any? # Checks if either price params passed
        if params.keys.include?('name') # Checks if attempting to search by price and name
        ...
        elsif ((params[:min_price].nil? || params[:min_price] == '') and params.keys.include?('min_price')) || ((params[:max_price].nil? || params[:max_price] == '') and params.keys.include?('max_price')) # Checks if either price param is empty
          ...
          send_error('Max price must be greater than or equal to Minimum Price', 400)
        else
        ...
        if params[:name].nil? || params[:name] == '' # Checks if name param is empty
